...
> 这里提一嘴组件声明的问题：我们在对自己封装的组件添加`name`标识的时候，需要在.vue 文件中定义两个`script` 标签，一个用来定义组件的`name`，一个用来编写组件逻辑。我想你肯定跟我一样，是不太能接受这种有点“多此一举”的写法的。不过我想找个问题`vue`官方应该会在后续解决掉的，这里我们使用一个大佬开发的插件来帮我们解决找个问题。
...
我们的项目时 TS 项目，所以也需要同时适配`tsconfig.json`
